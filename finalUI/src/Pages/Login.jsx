...
import { HeadIngs } from "../components/Headings"; // Assuming you have a 'Headings' component
import ForMs from "../components/ForMs"; // Assuming you have a 'ForMs' component
...
function LoGin() {
 ...
 const location = useLocation();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let resp =sendlogin(email, password);
    resp.then((data)=>{
    return(data.json())
    }).then((newdata)=>{
      console.log(newdata);
      const newPath = `${location.pathname}/redirect`;
      console.log(newPath);

      navigate(newPath);
      // navigate("http://localhost:5173/login/redirect");
    })
      ...
      <div className="lands landingpage flex align-center gap-[8.1rem] w-[80vw] h-[90%] pt-[0%] ml-[3%]">
        <div className="headings pt-[5%] flex-col w-[40%] h-inherit">
          ...
          <div className="flex gap-2 text-[14px] font-[400] justify-center">
            ...
            <Link className="text-blue-800 pt-2" to="/signup">
              Sign Up
            </Link>
          </div>
        </div>
