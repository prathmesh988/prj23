...
import { HeadIngs } from "../components/Heading2"; // Assuming you have a 'Heading2' component
import ForMs  from "../components/ForMs"; // Import 'ForMs' as a named import
import { sendaccount } from "../ServerReq";
...
function SignUp() {
  ...
  const [username, chagneusr] = useState('');

  const handleInputChange3 = (e) => {
    chagneusr(e.target.value);
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    let resp = sendaccount(email, username, password);
    resp.then((data) => {
      console.log(data);
      return data.json();
    }).then(
      (newdata) => {
        ...
        if(newdata.success===true){
          ...
          const newPath = `${location.pathname}/redirect2`;
          console.log(newPath);
    ...
    console.log('hello')
    console.log(resp);
    // Assuming 'sendlogin' requires email and password
  };
            ...
            <form onSubmit={handleSubmit} >
              ...
              <div  className="flex flex-col gap-2 pt-[0.2rem]">
                <div className="username inpboxesofform ">
                  <span className="pr-2 pl-3 material-symbols-outlined ">
                    <svg
                      width="14"
                      height="20"
                      viewBox="0 0 14 20"
                ...
                <ForMs changeemail={changeemail} changepwd={changepwd} className="w-[50%]" />
              </div>
              <p className="text-[12px]">Your password should have at least 8 characters.</p>
              <div className="flex gap-1 pt-1 items-start">
                <input
                  type="checkbox"
                  className="bordergraty2 overflow-hidden shrink-0 mt-[0.15rem] pl-[0.1rem]"
                />
                <div className="text-[12px] pl-1">
                  <p>
                    By creating an account, you agree to the <span className="font-[600]">Terms</span>
                  </p>
                  <p>
                    <span className="font-[600]">Conditions</span> and our Privacy Policy
                  </p>
                </div>
              </div>
              ...
              <div className="top-[50px] left-[260px] w-[100%] flex flex-row items-center justify-start gap-[16px] text-center text-xs">
                ...
                <div className="relative leading-[160%] p-2 text-[12px]">
                  Or sign up with
                </div>
                <div className="flex-1 relative box-border h-px border-t-[1px] border-solid border-greyscale-200" />
              </div>
