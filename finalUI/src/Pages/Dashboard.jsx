...
import { GetBalance ,GetOrdersByBatch, GetTotalDeposited ,GetOrders  , GetAllSalesHistory} from "../ServerReq";
...
function DashboardSection() {
   ...
   const [orders1 , changeorders1]=useState('')
   
   const [transectionhist , changehist]= useState('')

   //code for transiction history access to that->
   useEffect(()=>{
    GetAllSalesHistory().then((data)=>{
      return data.json()
    }).then((newdata)=>{

      changehist(newdata)
      console.log('the transiction history is',newdata.result)
      // console.log(newdata.result[123234].length)
      //you can add the  transicationhist  statevariable to the 
      // to transiction history component and then map it to it
    })
   },[])
   ...
   useEffect(()=>{
      GetOrdersByBatch().then((data)=>{
        return data.json()
      }).then((newdata)=>{
        changeorders(newdata.result[123234].length)
        // console.log()
      })
   },[])
   ...
   useEffect(()=>{
    GetOrders().then((data)=>{
      ...
      changeorders1(newdata.result[123234].length)
      // console.log()
    })
 },[])
  ...
  useEffect(()=>{
    GetTotalDeposited().then((data)=>{
      return data.json()
    }).then((newdata)=>{
      changeDeposit(newdata.result)
  ...
  return (
    <section className="flex-grow">
      ...
      <div className="flex">
      <section className="px-6 py-2" style={{ width: '-webkit-fill-available' }}>
          <section className="min-w-[50vw] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.14)] px-6 py-6 flex justify-between">
            <div className="">
              ...
              <div className="h-[60px] border-r-2 pt-4 flex align-bottom w-[200px]">
                <h1 className="font-bold text-lg self-end">${Amt}</h1>
              </div>
            ...
            <div className="">
              ...
              <div className="h-[60px] border-r-2 pt-4 flex align-bottom w-[200px]">
                <h1 className="font-bold text-lg self-end">{/*28*/ orders }</h1>
              </div>
              <p className="text-xs text-gray-500 font-bold">Last Week 0</p>
            </div>
            <div className="">
              ...
              <div className="h-[60px] border-r-2 pt-4 flex align-bottom w-[200px]">
                <h1 className="font-bold text-lg self-end">${deposit}</h1>
              </div>
            ...
            <div className="">
              ...
              <div className="h-[60px] pt-4 flex align-bottom w-[200px]">
                <h1 className="font-bold text-lg self-end">{orders1}</h1>
              </div>
              <p className="text-xs text-gray-500 font-bold">Last Week $0</p>
            </div>
          </section>
